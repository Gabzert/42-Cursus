...
static void	last_handler(t_stack *a, t_stack *b, int min, int max)
{
	...
	new_min = find_min_max(a, -1);
	if (b->array[0] != min && b->array[0] != max)
	{
		while ((a->array[0] < b->array[0] || a->array[a->size - 1]
				> b->array[0]) && b->size != 0)
			shift(a, 1);
	}
	else
...
void	small_sort(t_stack *a, t_stack *b)
{
	...
	push(a, b, -1);
	small_sort3(a);
	if (b->array[0] > a->array[0] && b->array[0] < a->array[a->size - 1]
		&& b->array[0] < a->array[1])
		shift(a, 1);
	else if (b->array[0] > a->array[0] && b->array[0] < a->array[a->size - 1]
		&& b->array[0] > a->array[1])
		rev_shift(a, 1);
	push(b, a, 1);
